...
<template>
        <!-- insert profile photo instead of svg -->
    <div class="relative cursor-pointer">
        <div v-if="imagePreviewUrl" @click="toggleFullScaleImage()">
        ...
        <label for="fileInput" class="cursor-pointer">
            ...
            <!-- default svg icon -->
            <svg v-if="!imagePreviewUrl" viewBox="-3.6 -3.6 31.20 31.20" fill="none" class="w-[100px] h-[100px]">
            ...
            <!-- input over big and small icon by ref "fileInput" -->
            <input ref="fileInput" id="fileInput" type="file" @input="pickFile" class="absolute opacity-0 pointer-events-none">

        </label>
    </div>

        <!-- open or close image in full scale -->
        <div v-if="fullScaleImage" class="absolute inset-0 flex justify-center items-center bg-black bg-opacity-50 z-50">
        ...
        <!-- close button -->
            <button @click="toggleFullScaleImage" class="absolute top-4 w-[64px] h-[64px]">
                <svg viewBox="-3.6 -3.6 31.20 31.20" fill="none"><g id="SVGRepo_bgCarrier" stroke-width="0">
                    <rect class="hover:fill-gray-400 transition duration-75" x="-3.6" y="-3.6" width="31.20" height="31.20" rx="15.6" fill="#d1d5db" strokewidth="0"></rect></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier">
